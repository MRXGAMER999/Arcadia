...
/**
 * Implementation of AIRepository for interacting with Google's Gemini AI.
 * Features:
 ...
 * - Injected GenerativeModel instances for efficient resource usage
 */
...
Use official studio names as they appear in game credits.
        ...
        val response = jsonModel.generateContent(prompt)
        val text = response.text?.trim()
            ?: throw Exception("Empty Gemini response for studio expansion")
...
Rules for slugs: lowercase, hyphenated, RAWG API compatible (e.g., "ryu-ga-gotoku-studio", "infinity-ward")
    ...
    private suspend fun performLibraryBasedRecommendations(
        ...
        return try {
            ...
            val libraryStr = buildEnhancedLibraryString(games)
            ...
            val prompt = GeminiPrompts.libraryBasedRecommendationPromptV3(libraryStr, exclusionList, count)

            Log.d(TAG, "Asking Gemini for library recommendations with ${games.size} games (full exclusion list)")

            val response = jsonModel.generateContent(prompt)
            val text = response.text?.trim()
                ?: return Result.failure(AIError.EmptyResponseError())

            Log.d(TAG, "Gemini response: ${text.take(500)}...")
    ...
    /**
     * Builds a compact library string for AI analysis.
     * Token-optimized format with developer/publisher frequency analysis.
     */
    private fun buildEnhancedLibraryString(games: List<GameListEntry>): String {
        ...
        // Developer frequency - key for recommendations
        ...
        // Compact single-line format per game
